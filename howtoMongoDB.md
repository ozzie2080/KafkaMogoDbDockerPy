...
The following steps to run MongoDB on Docker assumes basic knowledge about Docker and MongoDB

Notes on this example:
* MongoDB runs on a Docker container
* The container will continue running inthe background (option -d)
* The port used for MongoDB is kept the same and is made available from the container (option -p 27017:27017)
* Data (for Mongo) is kept on the local computer and not on the container.  We use volumens (option -v data:/data/db).  Mongo keeps the collections in the directory '/data/db'
* The final step is to verify that we have access to the data using Python
 

##### STEP 1. Initiate the MongoDB server on Docker

```shell

# Initiate MongoDB latest version 
$ docker run -d -p 27017:27017 -v data:/data/db mongo
bf029f7c36c1....

# Allows confirmation that run on the container and gives us the ID
$ docker ps
...
<details>
   <summary>view details of execution..</summary>
...
# Status and obtain container ID 
$ docker ps
CONTAINER ID        IMAGE               COMMAND                  CREATED             STATUS              PORTS                      NAMES
bf029f7c36c1        mongo               "docker-entrypoint.s…"   1 week ago         Up 2 weeks          0.0.0.0:27017->27017/tcp   pedantic_jones

# Inspecting the container ID (above).
...
##### STEP 2. Verify connectivy with MongoDB

```shell
# using the ID (docker ps) start a shell-bash session
# directly on the container. The result is a prompt (user root) in the container.
...
##### STEP 3. Once at the MongoDB prompt (>) verify connectivity 

More details on use of MongoDB CLI (shell) can be reviewed online at [mongodb user's manual](https://docs.mongodb.com/manual/mongo/)
...
##### STEP 4. Leave the container

To exit MongoDB use CTRL-D o type ``exit``.  Then, repeat the same steps to exit the container
...
# returning to the localhost (local computer)
$ 
```


##### STEP 5. Connect to the database using Python 3.x

In this step we assume that the Python package `PyMongo` is already installed.

For more details on the use of this package use the reference on W3School [PyMongo](https://www.w3schools.com/python/python_mongodb_getstarted.asp)
...
##### STEP 6. Accessing the MongoDB container from python (program) 

This step is very similar to STEP 5 above. This step is mainly to illustrate the creation of a program 
that connects and returns documents from a collection on MongoDB.

To run this program, execute the commandline `python test_pymongo.py`.  Then repeat the same steps to exit the container.
